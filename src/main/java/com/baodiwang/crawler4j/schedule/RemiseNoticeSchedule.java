...
public class RemiseNoticeSchedule {
     ...
     */
    @Scheduled(cron = "0 10 12 * * ? ")//,每小时执行一次 从10点30分开始,
        ...
        for(int i = 13 ;i<200;i++){
            ...
            if(!remiseNoticeList.isEmpty()){
                try{
                    ...
                    log.error("【抓取出让公告列表页数据】...............................批量插入数据发生异常："+e.getMessage(),e);
                }
            }else{
                log.info("【抓取出让公告列表页数据】...............................本次抓取的数据中都已存在，无需处理");
            }
            int sleepMinute = 1;
            log.info("【抓取出让公告列表页数据】第" + i + "次抓取数据条数：" + remiseNoticeList.size() + ",处理条数：" + count + ",休眠" + sleepMinute + "分钟");
            try {
                Thread.sleep(sleepMinute * 60 * 1000);
            } catch (InterruptedException e) {
                log.error("【抓取出让公告列表页数据】休眠" + sleepMinute + "分钟发生异常" + e.getMessage(),e);
            }
        }
